 ...
 *
 * Everything decision #8 and ADR-0023 say about ranking players is said here about teams and means
 * the same thing — total points with the bench paid a credit, then head-to-head, then a declared
 ...
 *
 * The bye is the bench one level up (ADR-0020), so it is what earns the credit here. `teamsOnByeIn`
 * already asks the question the credit needs asked — a team one player short is orphaned rather
...
export function computeTeamStandings(session: Session): readonly TeamStanding[] {
  ...
  const results = playedMatches(session).flatMap((match) =>
    match.teams
      ? [
          {
            ids: [match.teams.sideA],
            points: match.score.sideA,
            opponentPoints: match.score.sideB,
            against: [match.teams.sideB],
          },
          {
            ids: [match.teams.sideB],
            points: match.score.sideB,
            opponentPoints: match.score.sideA,
            against: [match.teams.sideA],
          },
        ]
      : [],
  );
