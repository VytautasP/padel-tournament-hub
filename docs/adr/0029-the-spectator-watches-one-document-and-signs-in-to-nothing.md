...
The alternative was a `readOnly` flag through the existing tabs, and it was rejected for what it
would have left standing: every control would still be one condition away from the screen, and the
route would still be holding the store that could perform it. As built, the spectator's half of the
app has no import path to an engine operation at all.
